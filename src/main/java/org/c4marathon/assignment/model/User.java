...
@Entity
@Table(name = "users")
// 한 사람이 여러 아이디를 만들 필요는 없다.
// 그럼 사람 한명이 하나만 가지고 있는 고유한 무엇인가가 필요하다.
// 주민등록번호를 받기에는 민감한 정보이므로, email을 받아서 unique하게 만들어주자.
// 닉네임을 써보자
public class User {
    @Id
    ...
    // 입력시 json 으로 들어오는 값을 무시하는 것이다.
    // json으로 변환할 때, 이 필드를 무시하도록 하는 것이다.
    @JsonIgnore
