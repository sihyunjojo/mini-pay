...
import org.c4marathon.CalculateRequest;
...
public class CalculateController {
    ...
    public ResponseEntity<?> calculate(@RequestBody CalculateRequest request) {
        if (request.getType().equals(RANDOM_CALCULATE)) {
            Calculate calculate = calculateService.calculateBalanceRandomlyByUser(request.getUserList().get(0), request.getUserList(), request.getTotalBalance());
            return ResponseEntity.ok().body(calculate);
        }  else if (request.getType().equals(EQUALLY_CALCULATE)) {
            Calculate calculate = calculateService.calculateBalanceEquallyByUser(request.getUserList().get(0), request.getUserList(), request.getTotalBalance());
            return ResponseEntity.ok().body(calculate);
        } else {
